...
app.get("/", async (req, res) => {
  try {
    const [rows] = await con.query("SELECT * FROM rooms");
    res.json(rows);
  } catch (err) {
...
app.get("/rooms/:id", async (req, res) => {
  ...
  try {
    const [roomsRows] = await con.query("SELECT * FROM rooms WHERE id = ?", [roomId]);
    ...
    console.error("Error querying room details:", err);
    res.status(500).json({ error: JSON.stringify(err) });
  }
});
...
app.get("/images", async (req, res) => {
  try {
    const [rows] = await con.query("SELECT id, image_url FROM rooms");
    res.json(rows);
  } catch (err) {
    console.error("Error querying rooms:", err);
    res.status(500).json({ error: JSON.stringify(err) });
